...
setwd("C:/Users/gobia/Desktop/desktop/BS")
...
library(stats)
  ...
  p=i/100 # relative probability of a positive jump
  ...
  sigma=0.2 # volatility term in black scholes model
  w=-1/2*alpha^2-lambda*(p/(alphap+1)-(1-p)/(alphan-1))
  n=1000
  T=100
  dedj.char<-function(mu,T,sigma,lambda,p,alphap,alphan) {
    w=-1/2*alpha^2-lambda*(p/(alphap+1)-(1-p)/(alphan-1))
  ...
  setwd("C:/Users/gobia/Desktop/desktop/BS/test")
#for(i in 1:17){  
  for(j in 1:10){
    for(m in 1:10){
      for(n in 1:10){
          jpeg(paste("pic",i,"-",j,"-",m,"-",n,".jpeg"))
          d <- characteristic_function_to_density(
            function(mu=0,T=maturities[1],sigma=0.2,lambda=j,p=m/10,alphap=n/10,alphan=n/10){
              w=-1/2*alpha^2-lambda*(p/(alphap+1)-(1-p)/(alphan-1))
              exp(complex(real=0,imaginary=mu*w*T)-1/2*mu^2*sigma^2*T+lambda*T*(p/(alphap-complex(real=0,imaginary=mu))-(1-p)/(alphan+complex(real=0,imaginary=mu))))
            }
            ,2^8,
          ...
          d
          plot(d$x, d$density, las=1,main=paste("pic",i))
...
d <- characteristic_function_to_density(
  function(mu=0,T=maturities[1],sigma=0.2,lambda=9,p=0.5,alphap=0.1,alphan=0.1){
    w=-1/2*alpha^2-lambda*(p/(alphap+1)-(1-p)/(alphan-1))
    exp(complex(real=0,imaginary=mu*w*T)-1/2*mu^2*sigma^2*T+lambda*T*(p/(alphap-complex(real=0,imaginary=mu))-(1-p)/(alphan+complex(real=0,imaginary=mu))))
  }
  ,2^8,
...
d
plot(d$x, d$density, las=1,main="pic of maturity 87 days")
curve(dnorm(x,1,2), add=TRUE)
...
d <- characteristic_function_to_density(
  function(mu=0,T=maturities[2],sigma=0.2,lambda=28,p=0.5,alphap=0.4,alphan=0.4){
    w=-1/2*alpha^2-lambda*(p/(alphap+1)-(1-p)/(alphan-1))
    exp(complex(real=0,imaginary=mu*w*T)-1/2*mu^2*sigma^2*T+lambda*T*(p/(alphap-complex(real=0,imaginary=mu))-(1-p)/(alphan+complex(real=0,imaginary=mu))))
  }
  ,2^8,
...
d
plot(d$x, d$density, las=1,main="pic of maturity 31 days")
curve(dnorm(x,2.5,1.2), add=TRUE)
 

d <- characteristic_function_to_density(
  function(mu=0,T=maturities[3],sigma=0.2,lambda=15,p=0.5,alphap=0.9,alphan=0.9){
    w=-1/2*alpha^2-lambda*(p/(alphap+1)-(1-p)/(alphan-1))
    exp(complex(real=0,imaginary=mu*w*T)-1/2*mu^2*sigma^2*T+lambda*T*(p/(alphap-complex(real=0,imaginary=mu))-(1-p)/(alphan+complex(real=0,imaginary=mu))))
  }
  ,2^8,
...
plot(d$x, d$density, las=1,main="pic of maturity 59 days")
curve(dnorm(x,175.8337,0.428), add=TRUE)
